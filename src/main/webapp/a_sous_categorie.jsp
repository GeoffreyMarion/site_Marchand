			...
			<div class="content">
				...
				<div class="container-fluid">
					...
					<div class="d-flex flex-wrap">
						<a href="a_sous_categorie?create=1"><button type="button"
						...
						<form class="col-5 d-flex flex-wrap" role="search" method="post"
							action=a_sous_categorie id="recherche">
							<input type="search"
						...
						<form class="form d-flex flex-wrap" method="post" action=a_sous_categorie>
										 <div class="form-group row">
                                            <div class="col-sm-9">
                                                <select class="form-control" name="s-cat">
                                                	<c:forEach items="${ListCat}" var="categorie">
                                                	<c:if test="${categorie.id_categorie == sous_categorie.categorie.id_categorie}">
                                                		<option value="${categorie.id_categorie}"selected>${categorie.titre}</option>
                                                	</c:if>
                                                	<c:if test="${categorie.id_categorie != sous_categorie.categorie.id_categorie}">
										...
										<button type="submit"
											class="btn btn-light waves-effect waves-light width-md" name="se-cat">Chercher</button>
									</form>
					</div>
					<div class="row">
						<br>
					...
					<div class="alert alert-success" role="alert">Une sous catégorie a été créé avec succès</div>
				</c:if>
				<c:if test="${edition == true}">
					<div class="alert alert-success" role="alert">Une sous catégorie a été édité avec succès</div>
				</c:if>
				<c:if test="${suppression == true}">
					<div class="alert alert-danger" role="alert">Une sous catégorie a été supprimé avec succès</div>
				</c:if>
								...
								<div class="card-box">
									...
									<form class="form-horizontal" method="post">
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<input type="text" class="form-control" id="simpleinput"
													value="${sous_categorie.titre}" name="e-titre">
											</div>
										</div>
										 <div class="form-group row">
									...
									<table class="table mb-0 text-bg-dark">
										...
										<tbody>
											<c:forEach items="${ListSCat}" var="sous_categorie">
												<tr>
													<th scope="row">${sous_categorie.id_sous_categorie}</th>
													<td>${sous_categorie.titre}</td>
													<td>${sous_categorie.categorie.id_categorie}-${sous_categorie.categorie.titre}</td>
													<td><a
														href="a_sous_categorie?edit=${sous_categorie.id_sous_categorie}"><button
																type="submit"
																class="btn btn-sm btn-icon btn-light waves-effect waves-light">
																<i class="mdi mdi-keyboard"></i>
															</button></a> <a
														href="a_sous_categorie?suppr=${sous_categorie.id_sous_categorie}"><button
																type="submit"
